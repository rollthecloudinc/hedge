...
export class Service {
  ...
  bounce = (path, options) => this.region({ path })
      .then(({ region }) => region.document())
      .then(({ regionDocument }) => fetch('https://' + regionDocument.origin + path, options))

  region = ({ path }) => getObject({ path: 'renewable-report/report' })
      .then(report => pickRegion({ path, report, service: this._document }))
...
export class Hedge {
  ...
  region = ({ path }) => this._service.region({ path })

}

export const hedge = ({ service }) => getObject({ path: 'services/' + service }).then(document => new Hedge({ service: new Service({ document }) }))

function pickRegion({ service, report, path }) {
  const availableRegions = service.regions.filter(r => filterRegions({ region: r, uri: path })).map(r => r.region);
...
function filterRegions({ region, uri }) {
  return typeof(uri) === 'undefined' || typeof(region.paths) === 'undefined' || region.paths.length === 0 || region.paths.filter(path => isMatch(uri ,path)).length > 0
}